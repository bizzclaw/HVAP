...
function EFFECT:Init(data)
	...
	elseif self.Ent:IsValid() and self.Ent.IsHVAPGround then
		...
		if self.Size >= 90 then 
			sound.Play("HVAP.Bullet.Impact.Large.AP", self.Pos, 1, 1, 160)	
		elseif self.Size > 45 then
			sound.Play("HVAP.Bullet.Impact.Medium.AP", self.Pos, 1, 1, 150)	
		elseif self.Size >= 20 then
			sound.Play("HVAP.Bullet.Impact.Small.AP", self.Pos, 1, 1, 140)	
		else
			sound.Play("HVAP.Bullet.Impact.Metal", self.Pos, 1, 1, 130)	
		end
		util.Decal( "ExplosiveGunshot", self.Pos-self.Normal, self.Pos+self.Normal )	
	else
		...
		sound.Play(sounds[mats[self.Mat]], self.Pos, 1, 1, 110)	
