	...
	if self:GetNWBool("shooting") then
		if !self:GetNWBool("overheated") then
			...
			if !self.sounds.loop:IsPlaying() then
				self.sounds.start:PlayEx(1,math.Clamp(self.SndPitch-2+(10*heatscl^3)+mod,0,255))
			end
			self.sounds.loop:PlayEx(1,math.Clamp(self.SndPitch-2+(10*heatscl^3)+mod,0,255))		
		else
