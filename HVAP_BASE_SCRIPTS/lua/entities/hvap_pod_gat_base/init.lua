...
function ENT:Initialize()
	...
	self:addSounds()
	self.SmokeTimer = (self.FireRate/2560)
...
function ENT:fireBullet(pos)
	...
	local pos2 = self.IsAimed and self.aircraft.camera:LocalToWorld(pos)+ self.aircraft:getCameraAngles():Forward()*self.ShootOffset.x or self.aircraft:LocalToWorld(pos)
	local ang = (self.IsAimed and self.aircraft:getCameraAngles() or self.aircraft:GetAngles())+Angle(math.Rand(-self.Spread,self.Spread),math.Rand(-self.Spread,self.Spread),0)
	...
	b:Spawn()
	self:AddOnRemove(b)
	self:MuzzleEffect(pos2, self.Caliber, ang, self.CurEffect <= CurTime() and 1 or 0)
...
function ENT:MuzzleEffect(pos, size, ang, i)
	if i > 0 then
		self.CurEffect = CurTime()+self.SmokeTimer
	end
