AddCSLuaFile( "hvap_bullet_api.lua" )
...
-----------------------------------damage
	...
	util.BlastDamage(self.Owner, self.Owner, tr.HitPos, self.Size*4, self.TissueDamage/8)	
	self:Remove()
end
...
function ENT:Airburst()
	...
	effectdata:SetScale(self.EffectSize)
	util.Effect("hvap_bullet_impact_api", effectdata )
	util.ScreenShake(pos, 10, 5, 0.4, self.Size*2 )
-----------------------------------damage
	util.BlastDamage(self.Owner, self.Owner, pos, self.Radius*4, self.TissueDamage/2.56)
	self:Remove()
end
...
-----------------------------------impact effect
	...
	util.Effect( "hvap_bullet_penetrate_api", effectdata )
...
-----------------------------------damage
	...
	if pr.Entity:IsWorld() or !pr.Entity.isHVAP then
		util.BlastDamage(self.Owner, self.Owner, pr.HitPos, self.Size*2.56, self.TissueDamage/8)	
	end
end
