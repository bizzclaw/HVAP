AddCSLuaFile( "hvap_bullet_penetrate_ap.lua" )
					//Sound,Impact
					//  1	 2     3     4    5     6     7
...
local sounds={
	...
	[5]={--metal
		Sound("hvap/bullet/impact/metal_1.wav"),
		Sound("hvap/bullet/impact/metal_2.wav"),
		Sound("hvap/bullet/impact/metal_3.wav"),
		Sound("hvap/bullet/impact/metal_4.wav"),
		Sound("hvap/bullet/impact/metal_5.wav"),
		Sound("hvap/bullet/impact/metal_6.wav"),
		Sound("hvap/bullet/impact/metal_7.wav"),
		Sound("hvap/bullet/impact/metal_8.wav")
	},
	[6]={--snow
...
self.Emitter 		= ParticleEmitter( self.Pos )
 ...
 function EFFECT:Dust()
	...
	for i=0, 1.28*self.Scale do -- main smoke out
		local Smoke = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 20,500*self.Scale) + VectorRand():GetNormalized()*64*self.Scale )
	...
	for i=0, self.Scale do -- main smoke in
		local Smoke = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self.Pos )
		if (Smoke) then
	...
	for i=0, 1.92*self.Scale do -- dust out
		local Smoke = self.Emitter:Add( "particle/particle_composite", self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 0,64*self.Scale) + VectorRand():GetNormalized()*5*self.Scale )
	...
	for i=0, 1.92*self.Scale do -- particles out
		local Debris = self.Emitter:Add( "effects/fleck_cement"..math.random(1,2), self.Pos )
		if (Debris) then
		Debris:SetVelocity ( self.DirVec*-1 * math.random(200,300*self.Scale) + VectorRand():GetNormalized() * 192*self.Scale )
	...
	for i=0, self.Scale do -- particles in
		local Debris = self.Emitter:Add( "effects/fleck_cement"..math.random(1,2), self.Pos )
		if (Debris) then
 ...
 function EFFECT:Dirt()
	...
	for i=0, 1.92*self.Scale do -- main smoke out
		local Smoke = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 20,500*self.Scale) + VectorRand():GetNormalized()*64*self.Scale )
		Smoke:SetDieTime( math.Rand( 1.28 , 2.56 )*self.Scale )
	...
	for i=0, 2.56*self.Scale do -- dust out
		local Smoke = self.Emitter:Add( "particle/particle_composite", self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 0,64*self.Scale) + VectorRand():GetNormalized()*5*self.Scale )
		Smoke:SetDieTime( math.Rand( .6 , 1.28 )*self.Scale )
	...
	for i=0, 2.56*self.Scale do -- particles out
		local Debris = self.Emitter:Add( "effects/fleck_cement"..math.random(1,2), self.Pos )
		if (Debris) then
		Debris:SetVelocity ( self.DirVec*-1 * math.random(200,300*self.Scale) + VectorRand():GetNormalized() * 192*self.Scale )
		Debris:SetDieTime( math.random( .8, 1.92) )
 ...
 function EFFECT:Sand()
	...
	for i=0, 6*self.Scale do
		local Smoke = self.Emitter:Add( "particle/particle_composite", self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 20,500*self.Scale) + VectorRand():GetNormalized()*100*self.Scale )
 ...
 function EFFECT:Metal()
	...
	for i=0, self.Scale do -- main smoke out
		local Smoke = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec*-1 * math.random( 20,500*self.Scale) + VectorRand():GetNormalized()*64*self.Scale )
		Smoke:SetDieTime( math.Rand( 1.28 , 2.56 )*self.Scale )
	...
	for i=0, 2.56*self.Scale do -- spark out
		...
		particle:SetStartSize( math.Rand(6, 8)*self.Scale ) 
	...
	for i=0, 1.92*self.Scale do -- spark in
		local particle = self.Emitter:Add( "effects/spark", self.Pos ) 
		if (particle) then 
		particle:SetVelocity( ((self.DirVec*0.75)+VectorRand()) * math.Rand(32, 320)*self.Scale ) 
		particle:SetDieTime( math.Rand(0.32, 0.42) ) 				 
 ...
 function EFFECT:Wood()
	...
	for i=0, self.Scale/2 do -- main smoke in
		local Smoke = self.Emitter:Add( "particle/smokesprites_000"..math.random(1,9), self.Pos )
		if (Smoke) then
		Smoke:SetVelocity( self.DirVec * math.random( 20,200*self.Scale) + VectorRand():GetNormalized()*64*self.Scale )
		Smoke:SetDieTime( math.Rand( 1.28 , 2.56 )*self.Scale )
		Smoke:SetStartAlpha( math.Rand( 80, 100 ) )
		Smoke:SetEndAlpha( 0 )
		Smoke:SetStartSize( 15*self.Scale )
 ...
 function EFFECT:Glass()
	for i=0, 2.56*self.Scale do
		local Debris = self.Emitter:Add( "effects/fleck_glass"..math.random(1,3), self.Pos )
		if (Debris) then
		Debris:SetVelocity ( self.PenVec * math.random(0,80)*self.Scale + VectorRand():GetNormalized() *self.Scale )
