AddCSLuaFile( "hvap_bullet_penetrate_api.lua" )
					//Sound,Impact
					//  1	 2     3     4    5     6     7
...
self.Emitter 		= ParticleEmitter( self.Pos )
 ...
 function EFFECT:Wood()
	...
	for i=0, 4*self.Scale do -- out
		local Debris = self.Emitter:Add( "effects/fleck_wood"..math.random(1,2), self.Pos+self.DirVec )
		if (Debris) then
		Debris:SetVelocity ( self.DirVec*-1 + VectorRand():GetNormalized() * 300*self.Scale )
 ...
 function EFFECT:Glass()
	for i=0, 2.56*self.Scale do
		...
		if (Debris) then
		Debris:SetVelocity ( self.PenVec*-1 * math.random(0,80)*self.Scale + VectorRand():GetNormalized() *self.Scale )
