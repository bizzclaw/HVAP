...
surface.CreateFont("hvap_seatswitcher_text", {
	font = "Helvetica",
	size = 14
})
...
hvap.hook("HUDPaint", "hvap_seatswitch_hudpaint", function()
	local screen = {w = ScrW(), h = ScrH()}
	local crt = CurTime()
	local pl = LocalPlayer()
	local seat = pl:GetVehicle()
	local self = seat.hvap_seatswitcher
	...
	for k,s in pairs(self.seats) do
		...
		if IsValid(s) and IsValid(s:GetPassenger()) then
			...
			draw.SimpleTextOutlined(
				k..": "..n, "hvap_seatswitcher_text", screen.w-pos.x+4,
				screen.h-pos.y+3-#self.seats*17+k*17, colorPlayers,
				TEXT_ALIGN_LEFT, 0, 1, colorBackground
			)
		else
			draw.SimpleTextOutlined(
				k..": Empty", "hvap_seatswitcher_text", screen.w-pos.x+4,
				screen.h-pos.y+3-#self.seats*17+k*17, colorPlayers,
				TEXT_ALIGN_LEFT, 0, 1, colorBackground
			)
		end
...
hvap.hook("CreateMove", "hvap_cl_seatswitch_centerview", function(md)
	local p = LocalPlayer()
