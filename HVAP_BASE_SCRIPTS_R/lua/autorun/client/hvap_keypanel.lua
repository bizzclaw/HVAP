...
surface.CreateFont("hvap_keypanel", {
	font = "Tahoma",
	size = 14
})
...
function panel:Init()
	...
	self.label:SetFont("hvap_keypanel")
	self.label:SetText("")
	self.label:SetSize(w/2,h)
	self.key = vgui.Create("DLabel",self)
	self.key:SetFont("hvap_keypanel")
...
function panel:setKey(k)
	self.key:SetText(hvap.key.name(k))
	self.keyNum=k
	self.oldkey=k
end
...
hvap.key.addHook("hvap_key_panel", function(key, pressed)
...
function panel:SetEdit(b)
	if b then
		...
		self.key:SetText(hvap.key.name(self.keyNum))
		current=nil
	end
	self.editing = b
end
...
hvap.hook("SpawnMenuOpen","hvap_key_panel_opensm", function()
	if IsValid(current) and current.editing then return false end
end)

hvap.hook("Initialize", "hvap_key_panel_initgm", function()
